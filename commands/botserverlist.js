...
module.exports = {
  ...
  callback: async({message,client,channel}) => {
    client.guilds.cache.map(async(guild) => {
        guildId=guild.id
        sChannel = await guild.channels.cache.filter(c => c.type === 'GUILD_TEXT')
        invite = await sChannel.first().createInvite({maxAge:0,maxUses:0,reason:"Invite created by ZEcobot."})
        // console.log({invite})
